<?php
	...
	$toolscsv = $ini['dirWin'].'/data/tools.csv';
	...
	while($i<count($toolslist)) {
		...
		if($toolslist[$i]['拡張子'] == "php")  $txt .= "<a onClick='read_tool_php(\"". $dirhtml.$toolslist[$i]['ファイル名']. "\", \"resulttab\")'>".$toolslist[$i]['ファイル名']."</a>" ;
		else $txt .= "<a href='". $dirhtml.$toolslist[$i]['ファイル名']. "'>".$toolslist[$i]['ファイル名']."</a>" ;
		$txt .= "　……　{$toolslist[$i]['説明']}<br>";
		$i++;

	}
	if($i != 1) echo_panel($toolslist[($i-1)]['種類'], $txt, "info");
	else echo "『/data/tools/』フォルダにはファイルがありません。<br>";
