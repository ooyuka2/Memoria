      ...
      <?php
							...
							<select class="form-control input-normal input-sm theme" name="theme2[]">
								<?php
									...
									for($i=1;$i<count($todo_keeper_theme);$i++) {
										if($todo[$_GET['p']]['時間管理テーマ']==$todo_keeper_theme[$i]['id']) {
	    ...
	    <?php
		    ...
		    for($i=1; $i<count($todo);$i++) {
		    	if($todo[$i]['top']==$_GET['p'] && $todo[$i]['level']!=1 && $todo[$i]['削除']==0) {
		    		$detail = str_replace('<br>', '&#10;',$todo[$i]['作業内容']);
		    		$nokidate = date('Y/m/d',  strtotime($todo[$i]['納期']));
		    		$startdate = date('Y/m/d',  strtotime($todo[$i]['開始予定日']));
		    		$finishdate = date('Y/m/d',  strtotime($todo[$i]['終了予定日']));
